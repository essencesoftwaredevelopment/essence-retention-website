...
function readLeadQueryInput(query) {
  const rawUrl = typeof query?.url === "string" ? query.url : "";
  const rawDomain = typeof query?.domain === "string" ? query.domain : "";

  if (rawUrl && rawDomain) {
    throw buildValidationError("Provide either a website url or domain, not both.");
  }
...
async function resolveLeadUrlQuery(query) {
  const { sourceParam, rawValue } = readLeadQueryInput(query);
  const normalizedUrl = await validateLeadUrl(rawValue);
  ...
  return {
    ...
    normalizedUrl,
    normalizedDomain,
    upstreamParamValue: sourceParam === "domain" ? normalizedDomain : normalizedUrl,
  };
}
